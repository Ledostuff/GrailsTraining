<%@ page import="firstproject.Registration" %>
...
<div class="fieldcontain ${hasErrors(bean: registrationInstance, field: 'race', 'error')} required">
	...
	<g:select id="race" name="race.id" from="${firstproject.Race.list()}" optionKey="id" required="" value="${registrationInstance?.race?.id}" class="many-to-one"/>
</div>
